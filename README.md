...
- **Checkpoints via peer storage**: an encrypted channel checkpoint rides with the peers that offer storage, no setup. Import the seed with peer storage, reconnect to those peers, and the wallet offers to recover from the newest checkpoint they return: with the bundled engine that closes the channels safely and returns the funds on-chain (resuming them where they were waits on the engine, beignet #462 and #463). Nothing fences the old device in this mode.
