...
A guardian is a small always-on service speaking the guardian protocol, not a Lightning node: each entry is `<64-hex x-only pubkey>@<http(s) URL>` and the URL has to be reachable from this Umbrel. Settings keeps however many you have entered so far, so a set can be collected one server at a time; a wallet needs all three before it can turn a guardian mode on.
