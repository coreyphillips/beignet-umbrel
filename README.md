...
- **Receive while offline**: a wallet pre-signs a book of fixed-amount vouchers with a sibling that stays online (its primary node, say), hands out one invoice per voucher, and is paid while its daemon is off; the sibling settles each payment at once and the money lands when the wallet is back. The manager reconciles with the sibling after every start. See [docs/FFOR.md](docs/FFOR.md). Needs engine 0.21.4 or newer; the controls stay hidden until the bundled engine carries the routes and honours the role switch.
