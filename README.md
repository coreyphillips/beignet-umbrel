...
- **Automatic Lightning-first receiving**: the ordinary Receive form prepares an invoice that stays payable while the wallet is closed, then credits it automatically on reopening. No voucher-book setup or manual return is needed. Requires the companion daemon API after beignet 0.21.8 and a primary that opts into settlement and bounded channel funding. The image version must be updated before releasing this feature. See [the release dependency](docs/FFOR.md#automatic-lightning-first-receiving).
