...
| Reverse swaps | `rec.swaps` | A liquidity provider the operator opted in serves Lightning to on-chain swaps (beignet #737, `BEIGNET_SWAPS` with the `BEIGNET_SWAP_*` fee and exposure caps): it holds the incoming payment, funds a contract to the payer's address from its own on-chain balance, settles once the payer claimed, refunds itself if they never do. Off by default; the caps are the engine's own defaults. Editable in the Edit dialog under the provider switch. The Overview tab's Swaps card (`GET /swaps/status`) shows what is committed right now against the caps, with the ledger's state counts per direction. The status reports the caps per direction but the daemon enforces one budget over both (it admits a swap against the whole ledger, unfiltered), so when both directions serve, the card prints the caps once over the sum of the two committed figures and each direction keeps its own as the breakdown. |
| Submarine swaps | `rec.swaps.submarine` | The other direction, for a provider that already serves reverse swaps (beignet #743, `BEIGNET_SWAP_SUBMARINE` with `BEIGNET_SWAP_CLAIM_SAFETY_BLOCKS` and `BEIGNET_SWAP_PAYMENT_MAX_FEE_PPM`): a wallet locks coins on chain to a contract this node can claim with the payment's preimage, the node pays the wallet's Lightning invoice only after those coins confirmed and under a deadline set back from the wallet's refund height by the claim margin, then claims the coins with the preimage the payment reveals. The fee, size and exposure caps are shared with the reverse direction. Off by default; a second checkbox under Reverse swaps in the Edit dialog. Every `swap:*` event lands in the Logs tab. |
